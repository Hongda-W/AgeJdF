...
class RayTomoDataSet(h5py.File):
	...
	def set_input_parameters(self, minlon, maxlon, minlat, maxlat, pers=np.array([]), data_pfx='raytomo_in_', smoothpfx='N_INIT_', qcpfx='QC_'):
		...
		self.attrs.create(name = 'data_pfx', data=data_pfx.encode('utf8'))
		self.attrs.create(name = 'smoothpfx', data=smoothpfx.encode('utf8'))
		self.attrs.create(name = 'qcpfx', data=qcpfx.encode('utf8'))
		return
	
	def run_smooth(self, datadir, outdir, datatype='ph', channel='ZZ', dlon=0.2, dlat=0.2, stepinte=0.5, lengthcell=0.3, alpha1=1000, alpha2=100, sigma=50,
		...
		data_pfx=self.attrs['data_pfx'].decode('utf8')
		smoothpfx=self.attrs['smoothpfx'].decode('utf8')
		...
		while (not create_group):
			try:
				...
				runid+=1
				continue
		group.attrs.create(name = 'comments', data=comments.encode('utf8'))
		...
		group.attrs.create(name = 'datatype', data=datatype.encode('utf8'))
		group.attrs.create(name = 'channel', data=channel.encode('utf8'))
	...
	def run_qc(self, outdir, runid=0, smoothid=0, isotropic=False, datatype='ph', wavetype='R', crifactor=0.5, crilimit=10., dlon=0.1, dlat=0.1, stepinte=0.25,
			lengthcell=0.2, alpha=500, beta=50, sigma=30, lengthcellAni=0.1, anipara=1, xZone=2, alphaAni0=1200, betaAni0=1, sigmaAni0=200,
		...
		maxlat=self.attrs['maxlat']
		smoothpfx=self.attrs['smoothpfx'].decode('utf8')
		qcpfx=self.attrs['qcpfx'].decode('utf8')
		if isotropic:
		...
		group.attrs.create(name = 'datatype', data=datatype.encode('utf8'))
		group.attrs.create(name = 'wavetype', data=wavetype.encode('utf8'))
...
# m.imshow(ls.shade(topodat, cmap=mycm2, vert_exag=1., dx=1., dy=1., vmin=-11000, vmax=-0.5))
	...
	def plot_residual(self, dataid, period): 
		"""                                                                 -- Hongda, JUN 2017
		Get redidual values of the tomography result
		=======================================================================================
		Input Parameters:
		dataid              - dataid (e.g. smooth_run_0, qc_run_0 etc.)
		period              - period
		...
		=======================================================================================
		"""
		res_arr = self[dataid+'/%g_sec'%( period )]['residual'].value
		tomo_residual = res_arr[:,7]
		avg = np.mean(tomo_residual)
		std = np.std(tomo_residual)
		rms = np.sqrt(np.mean(np.square(tomo_residual)))
		bins = np.arange(np.floor(tomo_residual.min()), np.ceil(tomo_residual.max())+0.1)
		plt.hist(tomo_residual, bins=bins, color='gray', normed=True)  # plt.hist passes it's arguments to np.histogram
		props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
		plt.text(0.65, 0.95, ' $\mathrm{N}=%d$\n $\mu=%.3f$\n $\sigma=%.3f$\n $\mathrm{rms}=%.3f$' %(len(tomo_residual),avg,std,rms), \
				 transform=plt.gca().transAxes, fontsize=14,verticalalignment='top', bbox=props)
		plt.xlim(np.floor(tomo_residual.min()), np.ceil(tomo_residual.max()))
		plt.title("Histogram of the residuals for %g sec"%(period), fontsize=14)
		plt.xlabel("residual (sec)", fontsize=14)
		plt.show()
		return
	
	def residual_per_station(self, dataid, period, dist_lim=1.):
		"""                                                                 -- Hongda, JUN 2017
		Get redidual values of the tomography result
		=======================================================================================
		Input Parameters:
		dataid              - dataid (e.g. smooth_run_0, qc_run_0 etc.)
		period              - period
		dist_lim            - smallest distance between 2 stations for them to be recognised 
								as different stations, in km.
		=======================================================================================
		"""
		res_arr = self[dataid+'/%g_sec'%( period )]['residual'].value
		tomo_residual = res_arr[:,8]
		lat_1 = res_arr[:,1]
		lon_1 = res_arr[:,2]
		lat_2 = res_arr[:,3]
		lon_2 = res_arr[:,4]
		staCord = np.array([]); staSqrSum = np.array([]); staN = np.array([])# Full station list, each station only appear once.
		staCord = np.array([lat_1[0],lon_1[0]]).reshape(2,1)
		staSqrSum = np.array([tomo_residual[0] **2 ])
		staN = np.array([1])
		staCord = np.append(staCord, np.array([lat_2[0],lon_2[0]]).reshape(2,1), axis=1)
		staSqrSum = np.append(staSqrSum, tomo_residual[0]**2)
		staN = np.append(staN, 1)
		for i in range(lat_1.size - 1):
			min_dist_1, ind_1 = get_min_dist(lat_1[i+1], lon_1[i+1], staCord)
			if min_dist_1 < dist_lim:
				staSqrSum[ind_1] += tomo_residual[i+1] ** 2
				staN[ind_1] += 1
			else:
				staCord = np.append(staCord, np.array([lat_1[i+1],lon_1[i+1]]).reshape(2,1), axis=1)
				staSqrSum = np.append(staSqrSum, tomo_residual[i+1]**2)
				staN = np.append(staN, 1)
			min_dist_2, ind_2 = get_min_dist(lat_2[i+1], lon_2[i+1], staCord)
			if min_dist_2 < dist_lim:
				staSqrSum[ind_2] += tomo_residual[i+1] ** 2
				staN[ind_2] += 1
			else:
				staCord = np.append(staCord, np.array([lat_2[i+1],lon_2[i+1]]).reshape(2,1), axis=1)
				staSqrSum = np.append(staSqrSum, tomo_residual[i+1]**2)
				staN = np.append(staN, 1)
		sta_rms = np.sqrt(staSqrSum/staN) 
		m=self._get_basemap(projection='lambert', geopolygons=None)
		stax, stay = m(staCord[1,:], staCord[0,:])
		my_cmap = pycpt.load.gmtColormap('./cv.cpt') 
		plt.scatter(stax, stay, c=sta_rms, cmap=my_cmap, s=100)
		ax = plt.gca()
		plt.title("RMS of residuals for %g sec per station"%(period), fontsize=18, y=1.05)
		divider = make_axes_locatable(ax)
		cax = divider.append_axes("bottom", size="2%", pad=0.05)
		cbar = plt.colorbar(orientation='horizontal',cax=cax)
		cbar.set_label('rms of misfit (sec)')
		plt.show()
		return
	
	def plot_vel_iso(self, projection='lambert', fastaxis=False, geopolygons=None, showfig=True, vmin=None, vmax=None, hillshade=True):
		...
		# x_max, _= m(236.4, 39.5) # only plot the oceanic part for JdF
		#cmap = colormaps.make_colormap({0.0:[0.1,0.0,0.0], 0.2:[0.8,0.0,0.0], 0.3:[1.0,0.7,0.0],0.48:[0.92,0.92,0.92],
		#    0.5:[0.92,0.92,0.92], 0.52:[0.92,0.92,0.92], 0.7:[0.0,0.6,0.7], 0.8:[0.0,0.0,0.8], 1.0:[0.0,0.0,0.1]})
		my_cmap = pycpt.load.gmtColormap('./cv.cpt')
		if vmin == None:
			vmin = np.nanmin(self.vel_iso)
			vmin = np.ceil(vmin*20.)/20.
		if vmax == None:
			vmax = np.nanmax(self.vel_iso)
			vmax = np.floor(vmax*20.)/20.
		im = m.pcolormesh(x, y, self.vel_iso, cmap=my_cmap, shading='gouraud', vmin=vmin, vmax=vmax, alpha=alpha)
		# im = m.pcolormesh(x, y, self.vel_iso, cmap=my_cmap, vmin=vmin, vmax=vmax, alpha=0.5)
	#         im=m.pcolormesh(x, y, self.pdens, cmap=my_cmap, shading='gouraud')
		cb = m.colorbar(im, "bottom", size="3%", pad='2%', format='%.2f')
		cb.set_label(self.datatype.decode('utf8')+' V (km/s)', fontsize=12, rotation=0)
		cb.set_alpha(1)
		cb.draw_all()
		ax = plt.gca() # only plot the oceanic part for JdF
		# ax.set_xlim(right=x_max)
		fig = plt.gcf()
		fig.suptitle(str(self.period)+' sec', fontsize=16,y=0.95)
		if fastaxis:
			try:
	...
	def plot_pdens(self, projection='lambert', fastaxis=False, geopolygons=None, showfig=True, z_min=None, z_max=None):
		"""Plot isotropic velocity
		"""
		m = self._get_basemap(projection=projection, geopolygons=geopolygons, hillshade=False)
		x, y = m(self.lonArr, self.latArr)
		#cmap = colormaps.make_colormap({0.0:[0.1,0.0,0.0], 0.2:[0.8,0.0,0.0], 0.3:[1.0,0.7,0.0],0.48:[0.92,0.92,0.92],
		#    0.5:[0.92,0.92,0.92], 0.52:[0.92,0.92,0.92], 0.7:[0.0,0.6,0.7], 0.8:[0.0,0.0,0.8], 1.0:[0.0,0.0,0.1]})
		my_cmap = pycpt.load.gmtColormap('./cv.cpt')
		im = m.pcolormesh(x, y, self.pdens, cmap=my_cmap, shading='gouraud', vmin=z_min, vmax=z_max)
	#         im=m.pcolormesh(x, y, self.pdens, cmap=my_cmap, shading='gouraud')
		cb = m.colorbar(im, "bottom", size="3%", pad='2%', format='%d')
		cb.set_label('Path density', fontsize=12, rotation=0)
		fig = plt.gcf()
		fig.suptitle(str(self.period)+' sec', fontsize=16)
		if fastaxis:
			try:
				self.plot_fast_axis(inbasemap=m)
			except:
	...
	def generate_corrected_map(self, dataid, glbdir, outdir, pers=np.array([]), glbpfx='smpkolya_phv_R_', outpfx='smpkolya_phv_R_'):
		...
		for per in pers:
			...
			for i in range(InglbArr[:,0].size):
				lonG=InglbArr[i,0]
				latG=InglbArr[i,1]
				phVG=InglbArr[i,2]
				for j in range(lonArr.size):
					lonR=lonArr[j]
					latR=latArr[j]
					phVR=vel_iso[j]
...
def get_min_dist(lat, lon, cord_arr): # add ability to return the position of the station with minimum distance
    """
        Calculate the minimum distance between a given station and any station from a list.
    """
    if not cord_arr.shape[0] == 2: raise TypeiError("The existing station coordinates list must have 2 rows.")
    if lat in cord_arr[0,:]:
        ind_1 = np.where(lat==cord_arr[0,:])
        lon_mask = cord_arr[1,:][ind_1]
        if lon in lon_mask:
            return [float(0.), ind_1[0][np.where(lon==lon_mask)]]
    min_dist = 40031.
    for i in range(cord_arr.shape[1]):
        if degrees2kilometers(locations2degrees(lat,lon,cord_arr[0,i],cord_arr[1,i])) < min_dist:
            min_dist = degrees2kilometers(locations2degrees(lat,lon,cord_arr[0,i],cord_arr[1,i]))
            ind = i
        else:
            continue
    return [min_dist, ind]
